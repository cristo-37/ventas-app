...
class Ventas(tk.Frame):
    ...
    def widgets(self):
        ...
        boton_agregar = tk.Button(lblframe1, text="Agregar Articulo", bg="#dddddd", font="sans 12 bold", command=self.registar)
        ...
        self.label_suma_total = tk.Label(frame2, text="Total a pagar: PEN 0", bg="#C6D9E3", font="sans 25 bold")
    ...
    def actualizar_total(self):
        ...
        for child in self.tree.get_children():
            ...
            total += subtotal
        self.label_suma_total.config(text=f"Total a pagar: PEN {total:.0f}")
    ...
    def abrir_ventana_pago(self):
        ...
        label_total = tk.Label(ventana_pago, bg="#C6D9E3", text=f"Total a pagar: PEN {self.obtener_total():.0f}", font="sans 18 bold")
        ...
        def calcular_cambio():
            try:
               ...
               if cambio < 0:
                   ...
                   return
               label_cambio.config(text=f"Vuelto: PEN {cambio:.0f}")
            except ValueError:
        ...
        try:
            ...
            try:
                ...
                for child in self.tree.get_children():
                    ...
                    producto = item[0]
                    precio = item[1]
                    cantidad_vendidad = int(item[2])
                    subtotal = float(item[3])
                    productos.append([producto, precio, cantidad_vendidad, subtotal])
                    
                    if not self.verificar_stock(producto, cantidad_vendidad):
                    ...
                    c.execute("INSERT INTO ventas (factura, nombre_articulo, valor_articulo, cantidad, subtotal) VALUES (?,?,?,?,?)",
                              (self.numero_factura_actual, producto, float(precio), cantidad_vendidad, subtotal))
                    
                    c.execute("UPDATE inventario SET stock = stock - ? WHERE nombre = ?", (cantidad_vendidad, producto))
                    
                conn.commit()
                messagebox.showerror("Exito", "venta registrada exitosamente")
                ...
                self.label_suma_total.config(text="Total a pagar: PEN 0")
                ...
                conn.rollback()
                messagebox.showerror("Error", f"Error al registar la venta: {e}")
            finally:
                conn.close()
        except ValueError:
            messagebox.showerror("Error", "Cantidad pagada no valida")   
    ...
    def generar_factura_pdf(self, productos, total, factura_numero, fecha):
        ...
        c.drawString(100, heifht - 50, f"Factura #{factura_numero}")
        
        c.setFont("Helvetica-Bold", 16)
        c.drawString(100, heifht - 70, f"Fecha #{fecha}")
        
        c.setFont("Helvetica-Bold", 12)
        c.drawString(100, heifht - 100, "Informacion de la venta")
        
        data = [["Producto","Precio", "Cantidad", "Subtotal"]] + productos
        table = Table(data)
        table.wrapOn(c, width, heifht)
        table.drawOn(c, 100, heifht - 200)
        
        c.setFont("Helvetica-Bold", 16)
        c.drawString(100, heifht - 250, f"Total a pagar: PEN {total:.0f}")
        
        c.setFont("Helvetica-Bold", 12)
        c.drawString(100, heifht - 400, f"Gracias por su compra, vuelva pronto")
    ...
    def mostar_numero_factura(self):
    ...
    def abrir_ventana_factura(self):
        ...
        tree_facturas.heading("#3", text="Cantidad")
